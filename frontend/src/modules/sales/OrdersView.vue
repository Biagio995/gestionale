...
async function load(): Promise<void> {
  loading.value = true;
  try {
    orders.value = await salesService.fetchOrders({
    ...
    <main class="page">
      ...
      <div class="toolbar">
        ...
        <select v-model="statusFilter" @change="load">
          ...
          <option v-for="s in ['DRAFT','CONFIRMED','IN_PROGRESS','FULFILLED','CANCELLED','CONVERTED']" :key="s" :value="s">
            {{ t(`sales.orders.statuses.${s}`) }}
          </option>
        </select>
        <button type="button" class="btn btn-ghost" @click="load">{{ t('sales.search') }}</button>
      </div>
      <div v-if="loading" class="empty">{{ t('app.loading') }}</div>
      <div v-else-if="!orders.length" class="empty">{{ t('sales.orders.empty') }}</div>
      ...
      <table class="data-table">
        ...
        <tbody>
          ...
          >
            ...
            <td>{{ order.customer_company_name || order.customer_name || '—' }}</td>
            <td>{{ t(`sales.orders.statuses.${order.status}`) }}</td>
